...
def movef(ecycle, s1, w81, spa1, spb1, yprev1, ynext1, s2,  w82, spa2, spb2, yprev2, ynext2):
    ...
    for count in range(0, int(ecycle)):
        ...
        # condition 3 motion is done and position is held
            ...
            print "Motion should be done, as step (%d) > sinsize (%d)" % (count1, sinsize1)
        ...
        if count < w82:     # notes same as servo #1 above
            ...
            print "Motion should be done, as step (%d) > sinsize (%d)" % (count2, sinsize2)
...
def setup():
    ...
    # test
    [servo1, servo2] = [Servo("vertical", 600, 1750), Servo("horizontal", 600, 2400)]
...
def loop():
    ...
    movef(event, 1, 100, 25, 5, svo1o, svo1c, 1, 100, 1, 15, svo2o, svo2c)
